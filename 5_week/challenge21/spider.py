...
def parse(url, body):
    ...
    for i in response.xpath("//li[contains(@class, 'col-12')]"):
        title = i.xpath("./div/h3/a/text()").extract_first().split()[0]
        update = i.xpath("./div/relative-time/@datetime").extract_first()
        results.append((title, update))
        n +=1
    print(n)
...
async def task(url):
    async with aiohttp.ClientSession() as session:
        ...
        # 调用 fetch 获得 HTML 页面 TODO
        body = await fetch(session, url)
        
        # 调用 parse 解析页面并将获得的数据存入results
        parse(url, body)
