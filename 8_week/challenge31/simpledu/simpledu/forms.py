...
class UserForm(FlaskForm):
    ...
    def validate_username(self, field):
        try:
            username = User.query.filter_by(username=self.username.data).first().username
            if username and not(username == self.username.data):
    ...
    def validate_id(self, field):
        try :
            id = User.query.get(self.id.data).id

            if id and not(id == self.id.data):
    ...
    def validate_email(self, field):
        try:
            email = User.query.filter_by(email = self.email.data).first().email
            if not(email == self.email.data) and email :
